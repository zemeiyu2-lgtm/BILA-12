const CACHE='bila16-v1-5';
self.addEventListener('install',e=>self.skipWaiting());
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!=='bila16-v1-5').map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
